...
if __name__ == "__main__":
    ...
    if not cfg.only_eval:
        ...
        model = Net(cfg.pose_net, ULIP_model=None, tensor_list=tensor_list)
        model = model.cuda()
        model.eval()

        checkpoint_path = os.path.join(cfg.ckpt_dir, 'epoch_' + str(cfg.test_epoch) + '.pt')
        logger.info("=> loading checkpoint from path: {} ...".format(checkpoint_path))
        gorilla.solver.load_checkpoint(model=model, filename=checkpoint_path,strict=True)
