...
class GeometricAwareFeatureAggregator(nn.Module):
    def __init__(self, cfg):
        ...
        for i in range(self.block_num):
            self.GAFA_blocks.append(GAFA_block(self.K[i], self.d_model)) #, self.num_experts
        
    def forward(self, kpt_feature, kpt_3d, pts_feature, pts):#, cat_id
        """
        Args:
        ...
        """
        for i in range(self.block_num):
            kpt_feature = self.GAFA_blocks[i](kpt_feature, kpt_3d, pts_feature, pts)#, cat_id
...
class GAFA_block(nn.Module):
    ...
    def forward(self, kpt_feature, kpt_3d, pts_feature, pts): #, cat_id
        ...
        # kpt_feature = torch.stack([self.experts[cat_id[i]](kpt_feature[i].unsqueeze(0)) 
        #                            for i in range(kpt_feature.size(0))], dim=0).squeeze(1)
...
class FrontDoorEncoder(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.cross_attn_layer = AttnLayer(cfg.AttnLayer)
        self.self_attn_layer = SelfAttnLayer(cfg.SelfAttnLayer)
        self.mlp_local = nn.Linear(cfg.feat_dim, cfg.feat_dim)
        self.mlp_global = nn.Linear(cfg.feat_dim, cfg.feat_dim)
        

        self.ln = nn.LayerNorm(cfg.feat_dim)
        
        self.aug_linear = nn.Linear(cfg.feat_dim,1)
        self.ori_linear = nn.Linear(cfg.feat_dim,1)
        self.sigmoid = nn.Sigmoid()
    
    def forward(self, local_feats, global_feats):
        
        # local - self
        local_feats_self = self.mlp_local(local_feats)
        local_feats_self = self.self_attn_layer(local_feats_self)
        
        # global - cross
        local_feats_cross = self.mlp_global(local_feats)
        
            
        cross_feats, attn = self.cross_attn_layer(local_feats_cross, global_feats)
        final_feats = self.ln(local_feats_self + cross_feats)

        aug_linear_weight = self.aug_linear(final_feats)
        ori_linear_weight = self.ori_linear(local_feats)
        aug_weight = self.sigmoid(aug_linear_weight + ori_linear_weight)
        final_feats = torch.mul(aug_weight, final_feats) + torch.mul((1-aug_weight), local_feats)
        
        return final_feats#,local_feats,cross_feats
...
class InstanceAdaptiveKeypointDetector(nn.Module):
    ...
    def forward(self, rgb_local, pts_local):
        ...
        batch_kpt_query, attn = self.attn_layer(batch_kpt_query, input_feature.transpose(1, 2)) 
...
class PoseSizeEstimator(nn.Module):
    def __init__(self):
        ...
        self.pose_mlp1 = nn.Sequential(
            nn.Conv1d(64+64+256, 512, 1),#64+64+256
            nn.ReLU(),
            nn.Conv1d(512, 256, 1),
            nn.ReLU(),
        )
    ...
    def forward(self, pts1, pts2, pts1_local):#, kpt_normal
        ...
        r = self.rotation_estimator(pose_feat) # pose_feat
