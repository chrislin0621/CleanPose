...
def compute_independent_mAP(final_results, synset_names, degree_thresholds=[360], shift_thresholds=[100], iou_3d_thresholds=[0.1], iou_pose_thres=0.1, use_matches_for_pose=True, logger=None, cat_id=-1):

    num_classes = len(synset_names)   # 7 6个类别+1个背景
    degree_thres_list = list(degree_thresholds) + [360]
    num_degree_thres = len(degree_thres_list)
    ...
    pose_aps = np.zeros((num_classes + 1, num_degree_thres, num_shift_thres))   # 一共8个，背景+6个类别+平均
    ...
    if logger is not None:
        ...
        logger.warning('5 degree, 2cm: {:.1f}'.format(
            pose_aps[cat_id, degree_thres_list.index(5), shift_thres_list.index(2)] * 100))
        logger.warning('5 degree, 5cm: {:.1f}'.format(
            pose_aps[cat_id, degree_thres_list.index(5), shift_thres_list.index(5)] * 100))

        logger.warning('10 degree, 2cm: {:.1f}'.format(
            pose_aps[cat_id, degree_thres_list.index(10), shift_thres_list.index(2)] * 100))
        logger.warning('10 degree, 5cm: {:.1f}'.format(
            pose_aps[cat_id, degree_thres_list.index(10), shift_thres_list.index(5)] * 100))
...
def evaluate(path, logger=None, cat_id=-1):
    synset_names = ['BG',  # 0
                    'bottle',  # 1
                    'bowl',  # 2
                    'camera',  # 3
