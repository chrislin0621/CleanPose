...
def draw(img, imgpts, axes, color, draw_gt=True):
    ...
    # draw axes
    if not draw_gt:
        img = cv2.line(img, tuple(axes[0]), tuple(axes[1]), (0, 0, 255), 3) # blue
        img = cv2.line(img, tuple(axes[0]), tuple(axes[3]), (255, 0, 0), 3) # red
        img = cv2.line(img, tuple(axes[0]), tuple(axes[2]), (255,255,0), 3) ## y last
