...
class TestDataset():
    def __init__(self, image_size, sample_num, data_dir, setting, dataset_name):
        ...
        if dataset_name == 'real':
            self.intrinsics = [591.0125, 590.16775, 322.525, 244.11084]
            result_pkl_list = glob.glob(os.path.join(self.data_dir, 'results', 'nocs_results','real_test', 'results_*.pkl'))
        elif dataset_name == 'camera':
            self.intrinsics = [577.5, 577.5, 319.5, 239.5]
            result_pkl_list = glob.glob(os.path.join(self.data_dir, 'results', 'nocs_results', 'val', 'results_*.pkl'))
    ...
    def __getitem__(self, index):
        ...
        with open(path, 'rb') as f:
            data = cPickle.load(f)
        image_path = os.path.join(self.data_dir, data['image_path'][5:]) # original
