...
def init():
    ...
    log_dir = os.path.join("log", exp_name + extra_name)

    cfg = gorilla.Config.fromfile(args.config)
    cfg.exp_name = exp_name
    cfg.extra = extra_name
    cfg.log_dir = log_dir
    ...
    os.makedirs(os.path.join(cfg.draw_dir,exp_name + extra_name, "ours"), exist_ok=True)
...
if __name__ == "__main__":
    ...
    name = cfg.exp_name + cfg.extra

    with open(cfg.test_dataset.tensor_list_dir, 'rb') as f:
        tensor_list = cPickle.load(f)


    # model
    ours_model = Net(cfg.pose_net, ULIP_model=None, tensor_list=tensor_list).cuda()
    ...
    with torch.no_grad():
        for i, data in tqdm(enumerate(dataloder)):
            ...
            ours_path = os.path.join(draw_dir, name, "ours", '{}_{}_bbox.png'.format(scene_id, image_id))
            ...
            for idx, RTs in enumerate(ours_result['gt_RTs']):
                ...
                draw_image_bbox_ours = draw(draw_image_bbox_ours, projected_bbox, projected_axes, (255, 255, 255),draw_gt=True)
            ...
            for idx in range(num_pred_instances): 
                ...
                transformed_bbox_3d = transform_coordinates_3d(bbox_3d, RTs)
                projected_bbox = calculate_2d_projections(transformed_bbox_3d, intrinsics)
                draw_image_bbox_ours = draw(draw_image_bbox_ours, projected_bbox, projected_axes, (0,255,0), draw_gt=False)
